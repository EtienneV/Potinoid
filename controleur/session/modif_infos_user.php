...
if(isset($_POST['mdp_change']) && ($_POST['mdp_change'] == 'ok') && isset($_POST['nouveau_mdp']) && isset($_POST['verif_mdp']))
{
  ...
  else if($resultat_test_mdp == 'ok')
  {
    ...
    $req = $bdd->prepare('UPDATE users 
              SET mdp=?
              WHERE id_user = ?');     
        $req->execute(array($nouveau_mdp, $id_user));

        $_SESSION['membre_mdp'] = $nouveau_mdp; // On actualise le mdp dans la session, pour rester authentifié
...
if(isset($_POST['verif']) && ($_POST['verif'] == 'ok'))
  {
      ...
      {
          if(1)//$_FILES['changer_avatar']['size'] != 0) // Si il y a une image A REVOIR !!!!!!!!
          {
              if ($_FILES['changer_avatar']['error'] <= 0)
              {
                      if (1)//$_FILES['changer_avatar']['size'] <= 2097152)
                      {
